import { useReauthMutation } from "../services/reauth";
...
const Persist = () => {
    ...
    const {token,persist,setAccessToken,logged}=useCurrentState()
    ...
    useEffect(()=>{
        if (ran.current === false){
            ...
            !token?.access && persist && logged  ? reAccess():setIsLoading(false)
        }
        return ()=>{
            ran.current=true
        }
